...
func begin_pull(stack: Haystack, point: Vector3, normal: Vector3) -> void:
	...
	_pull_preview.material_override = stack.straw_material(info.color)
...
func _physics_process(delta: float) -> void:
	...
	var speed := (SPRINT if sprinting else WALK) * (0.82 if held else 1.0) * speed_mult
