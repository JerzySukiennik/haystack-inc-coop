...
func _update_path(delta: float) -> void:
	var aim := _aim_point()
	_end = aim.point
	end_snap = find_socket(_end, start_snap.get("owner", null))
...
func _owner_of(node: Node) -> Node:
	while node:
		if node is ConveyorLine or node is SellMachine:
			return node
...
func find_socket(point: Vector3, exclude_owner: Object = null) -> Dictionary:
	var best := {}
...
func _place() -> void:
	...
	line.build(_flow_points(), flow_grounds)
