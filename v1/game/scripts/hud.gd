...
const PROMPTS := {
	"stack": "[LMB]  pull hay from the stack",
	"piece": "[LMB]  grab hay",
	"holding": "release [LMB] drop  ·  hold [RMB] throw  ·  [wheel] distance",
	"charging": "release [RMB] to throw",
}
...
func _ready() -> void:
	...
	_prompt = _label(20)
	_place(_prompt, Control.PRESET_CENTER, Vector2(-450, 34), Vector2(900, 30))
	_prompt.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	root.add_child(_prompt)

	_charge = ProgressBar.new()
	_place(_charge, Control.PRESET_CENTER, Vector2(-60, 20), Vector2(120, 6))
	...
	var sub := _label(15)
	sub.text = "pieces left in the stack"
	sub.modulate.a = 0.8
	box.add_child(sub)
	_pulled = _label(15)
	...
	_help = _label(14)
	_place(_help, Control.PRESET_BOTTOM_LEFT, Vector2(28, -46), Vector2(1000, 24))
	_help.text = "WASD move  ·  Shift sprint  ·  Space jump  ·  LMB grab / pull  ·  RMB throw  ·  Esc menu"
	_help.modulate.a = 0.85
