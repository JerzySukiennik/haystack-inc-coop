...
func _ready() -> void:
	...
	box.add_child(title)
	_money = _label(34)
	_money.add_theme_color_override("font_color", Color(0.62, 1.0, 0.5))
	...
	_inventory = _label(14)
	_place(_inventory, Control.PRESET_BOTTOM_RIGHT, Vector2(-360, -46), Vector2(330, 24))
	_inventory.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	_inventory.modulate.a = 0.85
...
func set_money(n: int) -> void:
	...
	_money.pivot_offset = Vector2(0, 22)
	_money.scale = Vector2(1.12, 1.12)
	create_tween().tween_property(_money, "scale", Vector2.ONE, 0.2)
...
func set_prompt(text: String, big_dot: bool) -> void:
	if text != _prompt.text:
		_prompt.text = text
		_prompt.modulate.a = 0.0
		create_tween().tween_property(_prompt, "modulate:a", 0.9, 0.12)
	create_tween().tween_property(_dot, "scale", Vector2.ONE * (2.0 if big_dot else 1.0), 0.1)
...
func set_inventory(has_remote: bool, meters: int, equipped: bool) -> void:
	if not has_remote:
		_inventory.text = ""
		return
	_inventory.text = ("[1]  Conveyor remote  ·  %d m" % meters) if not equipped else ("Conveyor remote in hand  ·  %d m" % meters)
