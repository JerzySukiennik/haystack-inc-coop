...
| `assets/audio/sell_coin_1.ogg … sell_coin_3.ogg` | real single coin dropped on a table with short bounce (0.65–0.75 s); _2 pitched 1.09x, _3 0.92x | one straw sold in the sell machine (random variant) |
| `assets/audio/purchase_1.ogg` | real jingling coin handful, stereo, 1.15 s (trimmed, fade-out) | shop purchase |
| `assets/audio/deny_1.ogg` | dull wooden knock/thunk, 0.3 s (low-passed 3.5 kHz) | can't afford |
| `assets/audio/carousel_slide_1.ogg`, `carousel_slide_2.ogg` | soft card slide on felt pitched to 0.85x, low-passed, 0.6 s | shop carousel moves to next item |
| `assets/audio/holo_start_1.ogg` | real mechanical switch click, 0.2 s | hologram remote on / start placing |
| `assets/audio/holo_cancel_1.ogg` | real switch click-clack (two clicks), 0.3 s | hologram cancel |
| `assets/audio/holo_place_1.ogg` | solid clunk: Kenney impactMetal_heavy + impactWood_heavy at -4 dB, 0.31 s | conveyor placed |
| `assets/audio/shop_door_bell_1.ogg` | small real bell, ring + 1.25 s decay (not auditioned — may read as a "ding"; optional) | opening the shop |
| `assets/audio/shop_music_loop.ogg` | 64.000 s calm acoustic guitar loop (seamless per author), stereo, +5.9 dB linear gain only, OGG q4, 0.8 MB | shop background music (enable **Loop**) |

## Audio (v2)

All v2 SFX: 44.1 kHz mono OGG Vorbis (oggenc q6), built from recorded foley (no synth tones). One-shots peak -1/-2 dBFS; loops seamless (equal-power tail->head crossfade, seam level checked ±1 dB) — enable **Loop** on every `*_loop.ogg`. Not auditioned by ear (chosen by source description + waveform/spectrum analysis).

| Path | What | Suggested use |
|---|---|---|
| `assets/audio/pitchfork_stab_1.ogg … pitchfork_stab_3.ogg` | (v2) 0.3–0.4 s crunchy stab: snow-covered-gravel footstep crunch + dry-leaf rustle attack (-7 dB) + low-passed Kenney knife chop (-13 dB) | pitchfork into hay/dirt (random variant, pitch 0.9–1.1) |
| `assets/audio/basket_collect_1.ogg … basket_collect_3.ogg` | (v2) 0.35–0.5 s short dry rustle + small wood creak/squeak at -11 dB (wicker feel) | straw tossed into basket |
| `assets/audio/basket_dump_loop.ogg` | (v2) 3.05 s continuous dry rustle (two rustle takes chained, looped, + half-length offset copy at -4 dB to fill gaps) | hay pouring out of basket (Loop) |
| `assets/audio/blower_loop.ogg` | (v2) 3.0 s steady fan/motor roar (recorded "machine" loop) + real air-vent rumble at -4 dB | leaf blower running (Loop) |
| `assets/audio/blower_start_1.ogg` | (v2) 1.65 s spin-up: same material with playback rate ramp 0.35→1.0 + fade-in, then 0.35 s at speed | blower trigger pressed, then crossfade into `blower_loop` |
| `assets/audio/vacuum_loop.ogg` | (v2) 2.3 s whiny motor loop (whine ~3.7 kHz over broadband noise) | vacuum suction running (Loop) |
| `assets/audio/vacuum_shoot_loop.ogg` | (v2) 2.06 s compressed-air hiss stream (steady part of an air-flow recording, forward+reversed chain) | air cannon / shoot mode (Loop) |
| `assets/audio/machine_arm_1.ogg … machine_arm_3.ogg` | (v2) 0.42 / 0.46 / 0.70 s real robot servo/actuator movement, 20 ms fade-in, 100 ms fade-out (**CC-BY 3.0**) | robotic arm moves |
| `assets/audio/machine_hum_loop.ogg` | (v2) 3.0 s low electric motor hum, quiet (-24 dBFS RMS) | running machines (Loop, 3D, short range) |
| `assets/audio/hopper_drop_1.ogg`, `hopper_drop_2.ogg` | (v2) 0.2 s metal latch clack + wooden flap body / lock clack + wood tick | hopper gate releases an item |
| `assets/audio/machine_place_1.ogg` | (v2) 0.36 s solid wood slam + metal slam (-6 dB) | machine set down on ground |
| `assets/audio/machine_pickup_1.ogg` | (v2) 0.9 s two real socket-wrench ratchet strokes | machine lifted/unbolted |
| `assets/audio/upgrade_1.ogg` | (v2) 1.32 s three ratchet clicks, then a metal tool clank + wood thump | machine upgrade purchased |
| `assets/audio/equip_1.ogg`, `equip_2.ogg` | (v2) 0.3–0.37 s wooden item handle + leather grip / small metal tool + leather grip | equip / switch tool |
