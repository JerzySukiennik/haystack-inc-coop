...
static func fork_out(stack: Haystack, point: Vector3, normal: Vector3, toward: Vector3, n: int) -> int:
	var got := 0
	for k in n:
		var jitter := Vector3(randf_range(-0.25, 0.25), randf_range(-0.2, 0.2), randf_range(-0.25, 0.25))
		var info := stack.begin_take(point + jitter)
		if info.is_empty():
			break
		var xf: Transform3D = info.transform
		var out := normal * 0.35 + Vector3.UP * 0.1
...
func _fork_out() -> void:
	...
	var got := n
	if Net.is_client():
		main.sync.req_fork.rpc_id(1, _point, _normal, toward)
	else:
		got = fork_out(main.haystack, _point, _normal, toward, n)
	if got > 0:
		player.pulled += got
