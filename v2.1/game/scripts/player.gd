...
func _update_pull(delta: float) -> void:
	...
	if pull_t >= 1.0:
		...
		if Net.is_client():
			_pull_stack.restore_take(_pull_info)
			_pull_info = {}
			sync.req_take.rpc_id(1, _pull_point, xf, true)
			_await_grab_until = Time.get_ticks_msec() + 1500
			_pull_cooldown = 0.25
			pulled += 1
...
func adopt_held(piece: HayPiece) -> void:
	if held == piece:
		return
	held = piece
	piece.local_driven = true
	hold_distance = HOLD_MIN + 0.3
	_fresh_until = Time.get_ticks_msec() + 900


func grab(piece: HayPiece, fresh := false) -> void:
	if Net.is_client():
		if int(piece.get_meta("holder", 0)) != 0:
			return
		sync.req_grab.rpc_id(1, piece.net_id)
		hold_distance = clampf(camera.global_position.distance_to(piece.global_position), HOLD_MIN, HOLD_MAX)
...
func drop() -> void:
	...
	if Net.is_client():
		if is_instance_valid(piece):
			piece.local_driven = false
			sync.req_release.rpc_id(1, piece.net_id, Vector3.ZERO, Basis(), Vector3.ZERO)
		return
	if is_instance_valid(piece):
		piece.set_held(null)
...
func release_throw() -> void:
	...
	if Net.is_client():
		var c_side := forward.cross(Vector3.UP).normalized()
		var c_charge := charge
		held = null
		charging = false
		charge = 0.0
		charge_changed.emit(0.0)
		piece.local_driven = false
		sync.req_release.rpc_id(1, piece.net_id, forward * power + Vector3.UP * power * 0.06 + velocity * 0.6, Basis(forward, c_side.cross(forward), c_side), forward * randf_range(-10.0, 10.0))
		Sfx.play_at("throw" if c_charge > 0.35 else "throw_soft", camera.global_position + forward, lerpf(-14.0, -4.0, c_charge))
		return
	drop()
...
func _update_held(delta: float) -> void:
	...
	if Net.is_client():
		var want_c := camera.global_basis * Basis.from_euler(HOLD_POSE)
		var k := 1.0 - exp(-delta * 18.0)
		held.global_transform = Transform3D(Basis(held.global_basis.get_rotation_quaternion().slerp(want_c.get_rotation_quaternion(), k)), held.global_position.lerp(target, k))
		if int(held.get_meta("holder", 0)) != Net.my_id() and Time.get_ticks_msec() > _fresh_until:
			held.local_driven = false
			held = null
		return
