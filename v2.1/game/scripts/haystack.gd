...
var needle_pos := Vector3.ZERO
var by_id := {}
var sync: Node
var _next_piece_id := 0
var needle_xf := Transform3D()
var needle_depth := 0.075
var needle_reveal_at := 6
var needle_taken := false
var needle_cover := 999
var _needle_visual: MeshInstance3D
var _needle_mat: StandardMaterial3D
const NEEDLE_LENGTH := 0.42
const NEEDLE_REVEAL_COVER := 6
const NEEDLE_COVER_RADIUS := 0.3
const NEEDLE_REACH := 0.55
...
func _build_mound() -> void:
	...
	core.mesh = _mound_mesh(SHELL_DEPTH, func(p: Vector3) -> Color:
		return Color(0.17, 0.12, 0.05).lerp(Color(0.24, 0.17, 0.07), clampf(_noise.get_noise_2d(p.x * 4.0, p.z * 4.0) + 0.5, 0.0, 1.0)))
...
func _build_fuzz() -> void:
	var count := SHELL_STRAWS + SKIRT_STRAWS
	...
	for i in count:
		...
		if i >= SHELL_STRAWS:
			...
			var er := edge_radius(a)
			var r := er * _rng.randf_range(0.8, 1.0) + pow(_rng.randf(), 2.0) * 0.35
			var out := Vector3(cos(a), 0, sin(a))
			var hmax := maxf(surface_height(out.x * er * 0.8, out.z * er * 0.8), 0.0)
			var y := pow(_rng.randf(), 2.2) * minf(hmax, 0.9) + 0.015
			mid = Vector3(out.x * r, y, out.z * r)
			var side := Vector3(-out.z, 0, out.x)
			dir = (side * _rng.randf_range(-1, 1) + out * _rng.randf_range(-0.6, 0.6) + Vector3.UP * _rng.randf_range(-0.05, 0.55)).normalized()
			depth = _rng.randf_range(0.0, 0.05)
			s = mid
...
func needle_material() -> StandardMaterial3D:
	if _needle_mat == null:
		_needle_mat = StandardMaterial3D.new()
		_needle_mat.albedo_color = Color(0.86, 0.88, 0.92)
		_needle_mat.metallic = 1.0
		_needle_mat.roughness = 0.18
	return _needle_mat
...
func _place_needle() -> void:
	...
	while tries < 500:
		tries += 1
		var s := _surface_point(rng)
		if s.y < 0.5 or s.y > 2.1:
			continue
		var n := surface_normal(s.x, s.z)
		var tangent := n.cross(Vector3(rng.randf_range(-1, 1), rng.randf_range(-0.3, 0.3), rng.randf_range(-1, 1))).normalized()
		needle_pos = s - n * needle_depth
		needle_xf = Transform3D(MeshKit.basis_along(tangent, NEEDLE_LENGTH).scaled_local(Vector3(0.55, 1.0, 0.55)), needle_pos - tangent * NEEDLE_LENGTH * 0.5)
		break
	_needle_visual = MeshInstance3D.new()
	_needle_visual.mesh = MeshKit.straw_mesh()
	_needle_visual.material_override = needle_material()
	_needle_visual.transform = needle_xf
	add_child(_needle_visual)
	_update_needle_cover()


func _update_needle_cover() -> void:
	if needle_taken:
		needle_cover = 0
		return
	var c := _cell_key(needle_pos)
	var cover := 0
	for dx in range(-1, 2):
		for dy in range(-1, 2):
			for dz in range(-1, 2):
				...
				if not _shell_cells.has(key):
					continue
				for idx: int in _shell_cells[key]:
					var d: float = _shell_depth[idx]
					if d < 0.0 or d >= needle_depth - 0.02:
						continue
					if _shell_pos[idx].distance_to(needle_pos) < NEEDLE_COVER_RADIUS:
						cover += 1
	if needle_cover == 999:
		needle_reveal_at = maxi(NEEDLE_REVEAL_COVER, int(cover * 0.45))
	var was_hidden := needle_cover > needle_reveal_at
	needle_cover = cover
	if was_hidden and cover <= needle_reveal_at:
		needle_revealed.emit()


func needle_exposed() -> bool:
	return not needle_taken and needle_cover <= needle_reveal_at


func take_material(info: Dictionary) -> Material:
	return needle_material() if info.get("needle", false) else straw_material(info.color)


func begin_take(point: Vector3) -> Dictionary:
	if needle_exposed() and point.distance_to(needle_pos) < NEEDLE_REACH + 0.2:
		needle_taken = true
		_needle_visual.visible = false
		if Net.is_online() and Net.is_host() and sync:
			sync.needle_changed()
		return {"needle": true, "transform": global_transform * needle_xf, "color": Color(0.86, 0.88, 0.92), "length": NEEDLE_LENGTH, "index": -1, "depth": needle_depth}
	if remaining <= 0:
		return {}
	...
	if not needle_taken and _shell_pos[idx].distance_to(needle_pos) < 0.6:
		_update_needle_cover()
	if Net.is_online() and Net.is_host() and sync:
		sync.shell_taken(idx)
	return info
...
func restore_take(info: Dictionary) -> void:
	if info.is_empty():
		return
	if info.get("needle", false):
		needle_taken = false
		_needle_visual.visible = true
		_update_needle_cover()
		if Net.is_online() and Net.is_host() and sync:
			sync.needle_changed()
		return
	...
	if Net.is_online() and Net.is_host() and sync:
		sync.shell_restored(idx, info.depth)
...
func finish_take(info: Dictionary, mesh_xf: Transform3D) -> HayPiece:
	var is_needle: bool = info.get("needle", false)
	if not is_needle:
		remaining -= 1
	...
	if is_needle:
		piece.make_needle(needle_material())
...
func spawn_piece(pos: Vector3, basis: Basis, color: Color, length: float, id := 0) -> HayPiece:
	var piece := HayPiece.new()
	piece.setup(_straw_mesh, straw_material(color), length)
	piece.net_color = color
	if Net.is_client():
		piece.make_replica()
		piece.net_id = id
	else:
		_next_piece_id += 1
		piece.net_id = _next_piece_id
	by_id[piece.net_id] = piece
	pieces_root.add_child(piece)
	piece.global_transform = Transform3D(basis.orthonormalized(), pos)
	pieces.append(piece)
	var pid := piece.net_id
	piece.tree_exited.connect(func() -> void:
		pieces.erase(piece)
		if by_id.get(pid) == piece:
			by_id.erase(pid)
			if Net.is_online() and Net.is_host() and sync:
				sync.piece_removed(pid))
	if not Net.is_client():
		_enforce_cap()
		if Net.is_online() and sync:
			sync.piece_spawned(piece)
	return piece


func set_needle_state(pos: Vector3, xf: Transform3D, taken: bool) -> void:
	needle_pos = pos
	needle_xf = xf
	needle_taken = taken
	if _needle_visual:
		_needle_visual.transform = xf
		_needle_visual.visible = not taken
	needle_cover = 999
	_update_needle_cover()


func taken_indices() -> PackedInt32Array:
	var out := PackedInt32Array()
	for i in _shell_depth.size():
		if _shell_depth[i] < 0.0:
			out.append(i)
	return out


func apply_shell_take(idx: int) -> void:
	if idx < 0 or idx >= _shell_depth.size() or _shell_depth[idx] < 0.0:
		return
	_shell_depth[idx] = -1.0
	var xf := _load_xf(idx)
	_shell.set_instance_transform(idx, Transform3D(Basis.from_scale(Vector3.ZERO), xf.origin))
...
func apply_shell_restore(idx: int, depth: float) -> void:
	if idx < 0 or idx >= _shell_depth.size():
		return
	_shell_depth[idx] = depth
	_shell.set_instance_transform(idx, _load_xf(idx))
