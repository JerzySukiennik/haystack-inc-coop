...
func _update_fill() -> void:
	if _fill == null:
		return
	var f := clampf(float(_count()) / maxf(capacity(), 1.0), 0.0, 1.0)
	_fill.visible = f > 0.0
	...
	if _collecting and _collect_cd <= 0.0:
		...
		if piece and piece.held_by == null and not piece.selling:
			if _count() >= capacity():
				...
				if Net.is_client():
					main.sync.req_absorb.rpc_id(1, piece.net_id, "basket")
					remote_count += 1
				else:
					tank.absorb(piece)
				...
				_update_fill()
			_collect_cd = 0.12
	if _pouring and _pour_cd <= 0.0 and _count() > 0:
		var cam := player.camera
		var fwd := -cam.global_basis.z
		var pos := cam.global_position + fwd * 0.9 + Vector3.DOWN * 0.25
		var vel := fwd * 2.2 + Vector3(randf_range(-0.4, 0.4), 0.6, randf_range(-0.4, 0.4)) + player.velocity * 0.5
		if Net.is_client():
			main.sync.req_emit.rpc_id(1, "basket", pos, vel)
			remote_count -= 1
		else:
			tank.emit_one(main.haystack, pos, vel)
		_pour_cd = 0.07
		_update_fill()
...
func prompt() -> Dictionary:
	...
	var warn := "The basket is full" if _count() >= capacity() and piece else ""
