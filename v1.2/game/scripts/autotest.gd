...
func _run() -> void:
	...
	p.ray.force_raycast_update()
	var dig_point := p.ray.get_collision_point()
	for n in [50, 150, 400]:
		while start_count - stack.remaining < n:
			var info := stack.begin_take(dig_point)
			if info.is_empty():
				break
			stack.remaining -= 1
		await _seconds(0.2)
		await _shot("03d_hole_%d" % n)
	print("[autotest] dug ", start_count - stack.remaining)
	p.primary_press()
	await _seconds(2.3)
