...
func _ready() -> void:
	...
	tool.placed.connect(func(line: ConveyorLine) -> void: belt_built += line.meters)
...
func _on_needle_sold(_at: Vector3) -> void:
	...
	_spawn_trophy()
	win_screen.open({"time": play_time, "pulled": player.pulled, "earned": total_earned, "belt": belt_built})
...
func _process(_delta: float) -> void:
	...
	var want := Input.MOUSE_MODE_VISIBLE if (menu_open or shop.is_open or win_screen.is_open) else Input.mouse_mode
...
func buy(id: String, amount := 1) -> bool:
	var it := Catalog.item(id)
	if it.is_empty():
		return false
	if it.has("requires") and int(levels.get(it.requires, 0)) <= 0 and int(machines.get(it.requires, 0)) <= 0 and not _has_placed(it.requires):
		hud.toast("You need the %s first" % Catalog.item(it.requires).name.to_lower(), "error")
		return false
	var price := next_price(id, amount)
	if price < 0:
		hud.toast("%s is already at max level" % it.name, "info")
		return false
	if money < price:
		hud.toast("You need $%d more" % (price - money), "error")
		return false
	money -= price
	var lvl := int(levels.get(id, 0))
	match it.kind:
		...
		"machine":
			...
			hud.toast("Bought a %s · press %d to place it" % [it.name.to_lower(), int(it.slot)], "success")
		"tool":
			levels[id] = lvl + 1
			if lvl == 0:
				hud.toast("Bought the %s · press %d to use it" % [it.name.to_lower(), int(it.slot)], "success")
			else:
				hud.toast("%s upgraded to level %d" % [it.name, lvl + 1], "success")
		"upgrade":
			levels[id] = lvl + 1
			hud.toast("%s · level %d" % [it.name, lvl + 1], "success")
	apply_stats()
	inventory_changed.emit()
	return true
