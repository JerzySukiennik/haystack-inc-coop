...
func _register_commands() -> void:
	...
	_reg("give", [
		{"name": "item", "options_fn": func(_t: PackedStringArray) -> Array: return ["money", "meters", "everything"] + Catalog.ORDER.filter(func(x: String) -> bool: return x != "belt_meters")},
		{"name": "amount", "optional": true, "options": ["1", "10", "100", "1000"]},
	], "Give money, belt meters, any tool, machine or upgrade level, or everything at max.", func(a: Array) -> String:
		var what: String = a[0]
		...
		match what:
			...
			"everything":
				for id in Catalog.ORDER:
					...
					match it.kind:
						"tool", "upgrade":
							main.levels[id] = Catalog.tier_count(id)
						"machine":
							main.machines[id] = int(main.machines.get(id, 0)) + 3
				main.conveyor_meters += 200
				main.money += 10000
			_:
				var it := Catalog.item(what)
				if it.is_empty():
					return "!Unknown item " + what
				match it.kind:
					"tool", "upgrade":
						main.levels[what] = clampi(int(main.levels.get(what, 0)) + amount, 0, Catalog.tier_count(what))
					"machine":
						main.machines[what] = int(main.machines.get(what, 0)) + amount
					"meters":
						main.conveyor_meters += amount
		main.apply_stats()
		main.inventory_changed.emit()
		return "Gave %s x%d" % [what, amount])

	_reg("level", [
		{"name": "item", "options_fn": func(_t: PackedStringArray) -> Array: return Catalog.ORDER.filter(func(x: String) -> bool: return Catalog.tier_count(x) > 0)},
		{"name": "level", "options": ["0", "1", "2", "3"]},
	], "Set a tool or upgrade to a level.", func(a: Array) -> String:
		var id: String = a[0]
		if Catalog.tier_count(id) == 0:
			return "!%s has no levels" % id
		main.levels[id] = clampi(int(_num(a[1], 0)), 0, Catalog.tier_count(id))
		main.apply_stats()
		main.inventory_changed.emit()
		return "%s is now level %d" % [id, main.levels[id]])
