...
func _shop_and_conveyor_test(p: Player, stack: Haystack) -> void:
	...
	await _seconds(0.4)
	main.shop.buy_primary()
	main.shop._buy("belt_meters", 4)
	await _seconds(0.6)
	await _shot("16_shop_meters")
	report["shop_bought"] = main.has_conveyor_remote and main.conveyor_meters == 5 and main.money == 100 - 15 - 15
	main.shop.close()
	await _seconds(0.4)
	...
	if not lines.is_empty():
		...
		var lp := line.points[line.points.size() - 1]
		var sp: Vector3 = m.intake_sockets()[0].pos
		report["first_line_snapped_to_intake"] = Vector2(lp.x - sp.x, lp.z - sp.z).length() < 0.2
		cam.queue_free()
		p.camera.current = true
		await _chain_test(p, stack, line)
...
func _chain_test(p: Player, stack: Haystack, first: ConveyorLine) -> void:
	var tool: ConveyorTool = main.tool
	main.conveyor_meters = 30
	var s0: Vector3 = first.points[0]
	var out_dir := -ConveyorLine.flat_dir(first.points[0], first.points[1])
	p.global_position = s0 + out_dir * 3.0 + Vector3(0, -0.8, -2.5)
	p.velocity = Vector3.ZERO
	await _frames(3)
	main.belt.equip("conveyor_remote")
	p.look_at_point(Vector3(s0.x, 1.0, s0.z) - out_dir * 0.4)
	await _frames(3)
	tool.primary()
	report["chain_start_snapped"] = not tool.start_snap.is_empty() and tool.start_snap.owner == first
	p.look_at_point(Vector3(s0.x, 0.0, s0.z) + out_dir * 5.0 + Vector3(0, 0, -2.0))
	await _seconds(0.8)
	report["chain_valid"] = tool.valid
	report["chain_flows_into_first"] = not tool._flow_forward
	await _shot("21_chain_hologram")
	var count0 := get_tree().get_nodes_in_group("conveyor").size()
	tool.primary()
	await _frames(3)
	var lines := get_tree().get_nodes_in_group("conveyor")
	report["chain_placed"] = lines.size() == count0 + 1
	var second: ConveyorLine = lines[lines.size() - 1]
	var money0: int = main.money
	for i in 3:
		stack.spawn_piece(second.points[1 + i] + Vector3(0, 0.3, 0), Basis.IDENTITY, Color(0.85, 0.7, 0.35), 0.45)
	var cam := Camera3D.new()
	main.add_child(cam)
	cam.global_position = s0 + Vector3(3.0, 5.0, -6.0)
	cam.look_at(s0)
	cam.current = true
	await _seconds(1.2)
	await _shot("22_chain_running")
	await _seconds(6.0)
	await _shot("22b_chain_joint")
	await _seconds(12.0)
	report["chain_sold"] = "%d/3" % (main.money - money0)
	var lost := 0
	for piece in stack.pieces:
		if piece.global_position.y < 0.6 and piece.global_position.distance_to(s0) < 4.0:
			lost += 1
	report["chain_dropped_near_joint"] = lost
	cam.queue_free()
	p.camera.current = true
	var mid := second.points[second.points.size() / 2]
	p.global_position = mid + Vector3(2.5, -0.8, 0.0)
	await _frames(3)
	p.look_at_point(mid + Vector3(0, -0.05, 0))
	await _frames(4)
	report["hover_line"] = tool.hover_line == second
	var first_pt := second.points[0]
	tool.rotate_action()
	await _frames(2)
	report["reverse_flips"] = second.points[second.points.size() - 1].distance_to(first_pt) < 0.01
	var meters0: int = main.conveyor_meters
	var refund := second.meters
	tool.secondary(true)
	await _seconds(1.0)
	tool.secondary(false)
	await _frames(3)
	report["dismantled_refund"] = main.conveyor_meters == meters0 + refund and not is_instance_valid(second)
	await _junction_test(p, stack, first)
	main.belt.equip("")


func _place_line(p: Player, from: Vector3, to: Vector3) -> ConveyorLine:
	var tool: ConveyorTool = main.tool
	p.global_position = (from + to) * 0.5 + Vector3(4.0, 0.2, -3.0)
	p.global_position.y = 0.2
	p.velocity = Vector3.ZERO
	await _frames(3)
	p.look_at_point(from)
	await _frames(3)
	tool.primary()
	p.look_at_point(to)
	await _seconds(0.7)
	...
	if not ok:
		...
		return null
	var lines := get_tree().get_nodes_in_group("conveyor")
	return lines[lines.size() - 1] if lines.size() > count0 else null


func _junction_test(p: Player, stack: Haystack, first: ConveyorLine) -> void:
	main.conveyor_meters = 60
	main.belt.equip("conveyor_remote")
	var total := first.length()
	var merge_at: Vector3 = first.point_at(total * 0.35).pos
	var merge := await _place_line(p, merge_at + Vector3(0.5, -1.0, -5.5), merge_at + Vector3(0, 0, -0.1))
	report["merge_placed"] = merge != null and not merge.feeds.is_empty()
	var split_at: Vector3 = first.point_at(total * 0.65).pos
	var branch := await _place_line(p, split_at + Vector3(0, 0, -0.1), split_at + Vector3(-0.5, -1.0, -5.5))
	report["split_placed"] = branch != null and first.splits.size() >= 1
	await _shot("23_junctions_built")
	if merge:
		var money0: int = main.money
		for i in 3:
			stack.spawn_piece(merge.points[1 + i * 2] + Vector3(0, 0.3, 0), Basis.IDENTITY, Color(0.85, 0.7, 0.35), 0.45)
		await _seconds(22.0)
		report["merge_sold"] = "%d/3" % (main.money - money0)
	if branch:
		var on_branch := 0
		var money_split0: int = main.money
		for i in 6:
			stack.spawn_piece(first.points[1] + Vector3(0, 0.3 + i * 0.05, 0), Basis.IDENTITY, Color(0.85, 0.7, 0.35), 0.45)
			await _seconds(1.2)
		await _seconds(6.0)
		for piece in stack.pieces:
			var pr := branch.project(piece.global_position)
			if not pr.is_empty() and pr.dist < 0.6 and piece.global_position.y > 0.7:
				on_branch += 1
		report["split_sold_of_6"] = main.money - money_split0
		report["split_diverted"] = on_branch
		var cam := Camera3D.new()
		main.add_child(cam)
		cam.global_position = split_at + Vector3(4.0, 4.0, -5.0)
		cam.look_at(split_at + Vector3(0, 0.5, -1.5))
		cam.current = true
		await _seconds(0.4)
		await _shot("24_split_running")
		cam.queue_free()
		p.camera.current = true


func _needle_test(p: Player, stack: Haystack) -> void:
	var c: GameConsole = main.console
	main.belt.equip("")
	report["needle_hidden_at_start"] = not stack.needle_exposed()
	print("[dbg] needle pos=", stack.needle_pos, " cover=", stack.needle_cover, " reveal_at=", stack.needle_reveal_at)
	report["needle_pulls_to_reveal"] = stack.needle_cover - stack.needle_reveal_at
	c.execute("/needle reveal")
	await _frames(4)
	print("[dbg] after reveal cover=", stack.needle_cover)
	report["needle_revealed"] = stack.needle_exposed()
	p.look_at_point(stack.needle_pos)
	await _frames(3)
	await _shot("25_needle_glint")
	p.primary_press()
	await _seconds(2.3)
	var held := p.held
	report["pulled_needle"] = held != null and held.is_needle
	await _shot("26_holding_needle")
	if held == null:
		p.input_enabled = true
		return
	var m: SellMachine = main.sell_machine
	var sock: Vector3 = m.intake_sockets()[0].pos
	p.global_position = sock + Vector3(2.2, -0.9, 0)
	p.velocity = Vector3.ZERO
	await _frames(2)
	p.drop()
	held.global_position = Vector3(sock.x + 0.2, 1.15, sock.z)
	held.linear_velocity = Vector3.ZERO
	await _seconds(4.0)
	report["win_screen"] = main.win_screen.is_open and main.won
	await _seconds(0.8)
	await _shot("27_win_screen")
	main.win_screen.close()
	await _seconds(0.5)
	report["trophy"] = main.get_node_or_null("Trophy") != null
	if not report["trophy"]:
		p.input_enabled = true
		return
	var cam := Camera3D.new()
	main.add_child(cam)
	var t: Node3D = main.get_node("Trophy")
	cam.global_position = t.global_position + Vector3(2.2, 2.0, 2.2)
	cam.look_at(t.global_position + Vector3(0, 1.2, 0))
	cam.current = true
	await _seconds(0.5)
	await _shot("28_trophy")
	cam.queue_free()
	p.camera.current = true
	p.input_enabled = true


func _teleport(p: Player, pos: Vector3, look: Vector3) -> void:
...
func _v2_test(p: Player, stack: Haystack) -> void:
	var belt: ToolBelt = main.belt
	belt.equip("")
	main.console.execute("/give everything")
	await _frames(3)
	report["v2_stats"] = is_equal_approx(p.pull_time, 0.5) and is_equal_approx(p.reach, 6.0) and is_equal_approx(ConveyorLine.speed, 3.0) and int(main.stat("sell_price")) == 5
	report["v2_hotbar_slots"] = belt.slots().size()
	main.shop.open()
	main.shop.set_category(Catalog.UPGRADES)
	await _seconds(0.8)
	await _shot("30_shop_upgrades")
	main.shop.set_category(Catalog.MACHINES)
	await _seconds(0.6)
	await _shot("31_shop_machines")
	main.shop.set_category("All")
	main.shop.close()
	await _seconds(0.3)

	await _teleport(p, Vector3(0.6, 0.1, 7.6), Vector3(0.3, 1.3, 4.6))
	belt.equip_slot(2)
	await _frames(3)
	var rem0 := stack.remaining
	belt.primary()
	await _seconds(0.9)
	await _shot("32_pitchfork")
	belt.primary_up()
	await _seconds(0.3)
	report["pitchfork_straws"] = rem0 - stack.remaining

	var target: HayPiece = null
	for piece in stack.pieces:
		if piece.global_position.y < 0.5 and piece.global_position.distance_to(p.global_position) < 5.0:
			target = piece
			break
	belt.equip_slot(3)
	var basket: BasketTool = belt.tools["basket"]
	if target:
		await _teleport(p, target.global_position + Vector3(0, 0.1, 2.0), target.global_position)
		belt.primary()
		await _seconds(0.5)
		belt.primary_up()
	report["basket_collected"] = basket.tank.count()
	await _shot("33_basket")
	belt.secondary(true)
	await _seconds(1.0)
	belt.secondary(false)
	report["basket_poured"] = basket.tank.count() == 0

	var spot := Vector3(12, 0.1, 8)
	await _teleport(p, spot, spot + Vector3(0, 0.2, -3))
	var spawned: Array[HayPiece] = []
	for i in 10:
		spawned.append(stack.spawn_piece(spot + Vector3(randf_range(-0.6, 0.6), 0.4 + i * 0.05, -2.0 + randf_range(-0.4, 0.4)), Basis.IDENTITY, Color(0.85, 0.7, 0.35), 0.45))
	await _seconds(1.0)
	var start_pos: Array[Vector3] = []
	for sp in spawned:
		start_pos.append(sp.global_position)
	belt.equip_slot(4)
	await _frames(2)
	belt.primary()
	await _seconds(0.8)
	await _shot("34_blower")
	await _seconds(0.6)
	belt.primary_up()
	var moved := 0.0
	for i in spawned.size():
		if is_instance_valid(spawned[i]):
			moved += spawned[i].global_position.distance_to(start_pos[i])
	report["blower_avg_push_m"] = snappedf(moved / spawned.size(), 0.1)

	await _teleport(p, spot + Vector3(0, 0, 4), spot + Vector3(0, 0.2, -2))
	for i in 12:
		stack.spawn_piece(spot + Vector3(randf_range(-0.5, 0.5), 0.3 + i * 0.04, randf_range(-1.5, 0.5)), Basis.IDENTITY, Color(0.85, 0.7, 0.35), 0.45)
	await _seconds(0.8)
	belt.equip_slot(5)
	var vac: VacuumTool = belt.tools["vacuum"]
	await _frames(2)
	belt.primary()
	await _seconds(2.5)
	await _shot("35_vacuum")
	belt.primary_up()
	report["vacuum_sucked"] = vac.tank.count()
	belt.secondary(true)
	await _seconds(0.6)
	belt.secondary(false)
	report["vacuum_shot_out"] = report["vacuum_sucked"] - vac.tank.count()

	belt.equip_slot(6)
	await _frames(2)
	belt.primary()
	await _seconds(0.6)
	report["shop_remote_opens"] = main.shop.is_open
	main.shop.close()
	await _seconds(0.3)

	var a := 0.4
	var edge := stack.edge_radius(a)
	var place := Vector3(cos(a), 0, sin(a)) * (edge + 0.9)
	await _teleport(p, place * 1.55 + Vector3(0, 0.1, 0) + Vector3(-sin(a), 0, cos(a)) * 3.0, place)
	belt.equip_slot(7)
	await _seconds(0.4)
	var puller_tool: MachineTool = belt.tools["auto_puller"]
	report["puller_ghost_valid"] = puller_tool._valid
	if not puller_tool._valid:
		print("[autotest] puller invalid: ", puller_tool._reason)
	await _shot("36_puller_ghost")
	var rem1 := stack.remaining
	belt.primary()
	await _seconds(6.0)
	report["puller_pulled"] = rem1 - stack.remaining
	var cam := Camera3D.new()
	main.add_child(cam)
	cam.global_position = place * 1.0 + Vector3(0, 3.5, 0) + Vector3(-sin(a), 0, cos(a)) * 4.0
	cam.look_at(place + Vector3(0, 1.0, 0))
	cam.current = true
	await _seconds(0.3)
	await _shot("37_puller_running")
	cam.queue_free()
	p.camera.current = true

	var hop_spot := Vector3(14, 0, -4)
	await _teleport(p, hop_spot + Vector3(-5, 0.1, 0), hop_spot)
	belt.equip_slot(8)
	await _seconds(0.4)
	belt.primary()
	await _frames(3)
	var hoppers := get_tree().get_nodes_in_group("machine").filter(func(m: Node) -> bool: return (m as Machine).id == "hopper" and not (m as Machine).preview)
	report["hopper_placed"] = hoppers.size() > 0
	if hoppers.size() > 0:
		var hop: Hopper = hoppers[0]
		for i in 6:
			stack.spawn_piece(hop.global_transform * Vector3(0, 1.8 + i * 0.1, -0.4), Basis.IDENTITY, Color(0.85, 0.7, 0.35), 0.45)
		await _seconds(5.0)
		report["hopper_fed_out"] = "%d in, %d out" % [hop.absorbed, hop.emitted]

	var bin_spot := Vector3(14, 0, -12)
	await _teleport(p, bin_spot + Vector3(-5, 0.1, 0), bin_spot)
	belt.equip_slot(9)
	await _seconds(0.4)
	belt.primary()
	await _frames(3)
	var bins := get_tree().get_nodes_in_group("machine").filter(func(m: Node) -> bool: return (m as Machine).id == "mini_seller" and not (m as Machine).preview)
	if bins.size() > 0:
		var money0: int = main.money
		for i in 3:
			stack.spawn_piece((bins[0] as Node3D).global_position + Vector3(0, 1.6 + i * 0.15, 0), Basis.IDENTITY, Color(0.85, 0.7, 0.35), 0.45)
		await _seconds(2.0)
		report["sell_bin_earned"] = main.money - money0
	await _shot("38_machines")

	belt.equip_slot(8)
	await _teleport(p, hop_spot + Vector3(-3, 0.1, 0), hop_spot + Vector3(0, 0.6, 0))
	var before := int(main.machines.get("hopper", 0))
	belt.secondary(true)
	await _seconds(1.0)
	belt.secondary(false)
	await _frames(3)
	report["machine_picked_up"] = int(main.machines.get("hopper", 0)) == before + 1
	await _shot("39_hotbar")
	belt.equip("")
	await _frames(2)
...
func _run() -> void:
	...
	await _console_test()
	await _v2_test(p, stack)
	await _needle_test(p, stack)
