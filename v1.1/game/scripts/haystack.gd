...
var _straws: Array[ArrayMesh] = []
var _straw_lengths: Array[float] = []
...
func _ready() -> void:
	...
	for i in STRAW_VARIANTS:
		var length := _rng.randf_range(0.34, 0.52)
		_straws.append(MeshKit.straw_piece(_rng, length))
		_straw_lengths.append(length)
...
func _build_fuzz() -> void:
	...
	_shell.mesh = MeshKit.straw_mesh()
	...
	for i in count:
		...
		_shell.set_instance_transform(i, Transform3D(MeshKit.basis_along(dir, length), p))
		...
		_shell.set_instance_color(i, base.darkened(shade * 0.45 + _rng.randf_range(0.0, 0.08)))
		_shell_pos[i] = mid
...
func _take_shell_straw(point: Vector3) -> void:
			...
			for dz in range(-1, 2):
				...
				for idx: int in _shell_cells[key]:
					if _shell_depth[idx] < 0.0:
						continue
					var score: float = _shell_pos[idx].distance_to(point) + _shell_depth[idx] * 1.5
					if score < best_score:
						best_score = score
						best = idx
	if best >= 0:
		_shell_depth[best] = -1.0
		_shell.set_instance_transform(best, Transform3D(Basis.from_scale(Vector3.ZERO), _shell_pos[best]))
...
func spawn_piece(pos: Vector3, basis: Basis = Basis.IDENTITY) -> HayPiece:
	...
	var v := _rng.randi() % _straws.size()
	piece.setup(_straws[v], _hay_mat, _straw_lengths[v])
...
func preview_mesh() -> ArrayMesh:
	return _straws[_rng.randi() % _straws.size()]


func preview_material() -> Material:
	return _hay_mat


func pull_piece(point: Vector3, normal: Vector3, xf := Transform3D()) -> HayPiece:
	if remaining <= 0:
		return null
	remaining -= 1
	count_changed.emit(remaining)
	var pos := point + normal * 0.25
	pos.y = maxf(pos.y, 0.2)
	var basis := Basis.from_euler(Vector3(0, _rng.randf() * TAU, _rng.randf_range(-0.3, 0.3)))
	if xf != Transform3D():
		pos = xf.origin
		basis = xf.basis
	var piece := spawn_piece(pos, basis)
	_take_shell_straw(point)
	_burst(point, normal)
