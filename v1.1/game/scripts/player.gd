...
func begin_pull(stack: Haystack, point: Vector3, normal: Vector3) -> void:
	...
	_pull_preview.mesh = stack.preview_mesh()
	_pull_preview.material_override = stack.preview_material()
...
func _update_pull_preview() -> void:
	...
	var dir := (_pull_normal * 0.55 + right * 0.85 + Vector3.UP * 0.15).normalized()
	var side := dir.cross(Vector3.UP)
	if side.length() < 0.01:
		side = Vector3.RIGHT
	side = side.normalized()
	var wobble := sin(pull_t * 40.0) * 0.004 * (1.0 - pull_t)
	var out := lerpf(-0.12, 0.22, ease(pull_t, 1.8))
	_pull_preview.global_transform = Transform3D(Basis(dir, side.cross(dir).normalized(), side), _pull_point + dir * out + side * wobble)
...
func _update_pull(delta: float) -> void:
	...
	if pull_t >= 1.0:
		...
		_pull_preview.queue_free()
		pull_progress.emit(0.0)
		var piece := _pull_stack.pull_piece(_pull_point, _pull_normal, xf)
		if piece:
			_pull_cooldown = 0.25
			pulled += 1
