...
func _draw_suggestions() -> void:
	...
	for i in _suggestions.size():
		...
		sb.bg_color = Color(0.12, 0.10, 0.06, 0.92) if i == _selected else Color(0, 0, 0, 0.7)
		...
		var l := Label.new()
		l.text = _suggestions[i].label
		l.add_theme_font_size_override("font_size", 16)
		var col := ACCENT if i == _selected else (Color(1.0, 0.72, 0.55) if _suggestions[i].get("fix", false) else DIM)
		l.add_theme_color_override("font_color", col)
		row.add_child(l)
