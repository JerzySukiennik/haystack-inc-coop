...
func _build_intake(axis: Vector3) -> void:
	...
	belt.constant_linear_velocity = -axis * BELT_SPEED
	var shape := BoxShape3D.new()
	...
	sh.code = BELT_SHADER
	mat.shader = sh
	mat.set_shader_parameter("speed", -BELT_SPEED)
...
func _build_sign() -> void:
	...
	for axis in [Vector3(1, 0, 0), Vector3(0, 0, -1)]:
		...
		label.position = a * (h + 0.02) + Vector3.UP * 1.95
		label.rotation.y = atan2(a.x, a.z)
		add_child(label)
