...
func primary(pressed: bool) -> void:
	...
	if not _valid:
		Sfx.play_ui("deny", -8.0)
		return
	var m := Machine.make(id)
	m.main = main
	main.add_child(m)
	m.global_transform = _ghost.global_transform
	main.machines[id] = count() - 1
	main.inventory_changed.emit()
	Sfx.play_at("machine_place", m.global_position, -2.0)
	...
	if _picking:
		if not is_instance_valid(_hover):
			...
			if _pick_t >= 1.0:
				...
				main.machines[mid] = int(main.machines.get(mid, 0)) + 1
